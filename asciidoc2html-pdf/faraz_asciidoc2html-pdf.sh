...
# @author	Faraz-Ur-Rehman <fsharif@tssg.org>
...
JDIR="$(cygpath -m -a $(dirname $(readlink -f $0)))"
JINPATH="$(cygpath -m -a $(dirname $(readlink -f $2)))"
else
JDIR=$DIR
JINPATH=$INPATH
fi
...
java -jar $JDIR/bin/coverpagehelper.jar -c $JINPATH/$infile $output/inputReport/$outputfilename.output.xml

touch $output/inputReport/$outputfilename.txt

java -jar $JDIR/bin/coverpagehelper.jar -h $output/inputReport/$outputfilename.output.xml $output/inputReport/$outputfilename.txt

cat $INPATH/$infile >> $output/inputReport/$outputfilename.txt

cp $DIR/conf/template_stylesheet.css $output/inputReport

#Calls the asciidoc tool through the a2x toolchain to generate an output document in XML format
$DIR/bin/asciidoc-8.6.6/a2x.py -v --stylesheet=template_stylesheet.css -f xhtml -d article -L -D $output $output/inputReport/$outputfilename.txt
...
java -jar $JDIR/bin/coverpagehelper.jar -c $JINPATH/$infile $output/inputReport/$outputfilename.xml

#Calls the asciidoc tool to generate an output document in XML format
$DIR/bin/asciidoc-8.6.6/asciidoc.py --backend docbook  --doctype article  --out-file $output/inputReport/$outputfilename.output.xml $INPATH/$infile

# Calls the coverpagehelper.jar to copy the coverpage.xml contents into the index.xml output from asciidoc.
java -jar $JDIR/bin/coverpagehelper.jar -p $output/inputReport/$outputfilename.xml $output/inputReport/$outputfilename.output.xml

# Converts the XML using XSL stylesheets to a .fo file suitible for passing to tools such as Apache-FOP
xsltproc -nonet \
-param header.image.filename "'$JDIR/conf/whitepaper/template/tssglogo.png'" \
-param footer.image.filename "'$JDIR/conf/whitepaper/template/tssg_bar_full.png'" \
-param cover.image.filename "'$JDIR/conf/whitepaper/template/tssg_logo.png'" \
-param navig.graphics.path "'$JDIR/bin/asciidoc-8.6.6/images/icons/'" \
-param admon.graphics.path "'$JDIR/bin/asciidoc-8.6.6/images/icons/'" \
-param callout.graphics.path "'$JDIR/bin/asciidoc-8.6.6/images/icons/callouts/'" \
-o $output/inputReport/index.fo $JDIR/conf/template_stylesheet.xsl $output/inputReport/$outputfilename.output.xml
